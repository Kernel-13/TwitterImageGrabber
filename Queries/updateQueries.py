...
def update_user(user, tweet_id=None, tweet_date=None, status=None):
	...
	if SQ.get_user(user) is None:
		raise Exceptions.UnknownUser("We couldn't find any user with the name [ {} ] in the database!".format(user))

	db = DB.database()

	if tweet_id and tweet_date:
	...
	if status:
		if status.lower().startswith('a'): 		status = 'Active'
			...
			raise Exceptions.UnknownStatus("Status not recognized: {}".format(status))
...
def fuse_users(user_one, user_two):
	...
	if SQ.get_user(user_one) is None or SQ.get_user(user_two) is None:
		raise Exceptions.UnknownUser("Both or one of the users provided are not present in the database!".format(user))
		...
		for file in files:
			...
			if user_one.lower() in file.lower():
				...
				try:
					...
					renamed_file = "[Duplicate] " + file.lower().replace(user_one.lower(), user_two)
					os.rename(old_file, new_file)
