    ...
    while len(tweets) > 0:
        for twt in tweets:
            try:
                ...
                if options['max_dups'] != 0 and max_dups >= options['max_dups']:
                    print('\nDone    ---    Reached the max. number of duplicate tweets (' + str(max_dups) + ')')
                    return
...
def print_help(program_name):
    help_text = '\nUsage:\n'
    help_text += '    ' + program_name + ' --keys <credentials_file> --user-timeline <user> [--no-retweets] [--no-replies] [--max-dups N] [--no-video] [--no-image]\n'
    help_text += '    ' + program_name + ' --keys <credentials_file> --my-timeline [--no-replies] [--max-dups N] [--no-video] [--no-image]\n'
    ...
    help_text += '    ' + '{:<35}'.format('-nimg, --no-image') + 'Prevents images from being downloaded (Only videos/gif are downloaded)\n'
    ...
    help_text += '    ' + '{:<35}'.format('-mxd, --max_dups <N>') + 'Stops the download after it finds more than N duplicate tweets\n'
...
def main(argv):
    ...
    while count < len(argv):
        if argv[count] in ("-h", "--help"):
            ...
            count += 1
            options['max_dups'] = int(argv[count])
        else:
